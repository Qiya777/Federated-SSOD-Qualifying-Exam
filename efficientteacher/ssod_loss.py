...
#             t = np.array(t.cpu())
#             # original logic
#             if t[8] >= self.ignore_thres_high[int(t[1])]:
...
#             elif t[8] >= self.ignore_thres_low[int(t[1])]:
#                 if self.pseudo_label_with_obj:
#                     uncertain_targets.append(np.concatenate((t[:6], t[7:8])))
...
#         return reliable_targets, uncertain_targets, uncertain_obj_targets, uncertain_cls_targets
    def select_targets(self, targets):
        """
        targets: (N,9) = [img, cls, x, y, w, h, c6, c7(obj), c8(final/cls)]
        """
        import numpy as np
        import torch
    
        device = targets.device
        conf_col = targets.shape[1] - 1   # ✅最后一列（你现在是8）
        obj_col = 7                       # ✅你原逻辑里 obj_conf 用第7列
    
        reliable_targets = []       # ✅返回6列
        uncertain_targets = []      # ✅返回7列（最后一列score，供 with_pseudo_score=True）
        uncertain_obj_targets = []  # ✅返回7列
        uncertain_cls_targets = []  # 保持空：论文一致版中 uncertain 不参与 cls
    
        for t in targets:
            t = np.array(t.detach().cpu())
            cls_id = int(t[1])
            conf = float(t[conf_col])
    
            # 可靠：用 conf 判定，但返回6列（不给score）
            if conf >= self.ignore_thres_high[cls_id]:
                reliable_targets.append(t[:6])
    
            # 不确定：用 conf 判定，返回7列（score 用 obj 或 conf）
            elif conf >= self.ignore_thres_low[cls_id]:
                if self.pseudo_label_with_obj:
                    score = float(t[obj_col])  # uncertain 的 soft obj target 优先使用 obj_conf
                else:
                    score = conf

                row7 = np.concatenate((t[:6], [score]))
                uncertain_targets.append(row7)

                # 论文一致版：只有 obj_conf 足够高的 uncertain 才参与 bbox 分支
                if self.pseudo_label_with_bbox and float(t[obj_col]) >= 0.99:
                    uncertain_obj_targets.append(row7)
    
        # ✅循环结束后再打印一次（最终统计）
        if not hasattr(self, "_dbg_sel_once"):
            self._dbg_sel_once = True
            print("[SSOD_LOSS.select_targets] in targets:", tuple(targets.shape),
                  "conf_col=", conf_col,
                  "reliable=", len(reliable_targets),
                  "uncertain=", len(uncertain_targets),
                  "uc_obj=", len(uncertain_obj_targets),
                  "uc_cls=", len(uncertain_cls_targets))
            # 顺便打印阈值（看是不是过高）
            try:
                print("[SSOD_LOSS.select_targets] th_high head:", self.ignore_thres_high[:6],
                      "th_low head:", self.ignore_thres_low[:6])
            except Exception:
                pass
    
        # 转 tensor（空就给正确形状的空 tensor）
        if len(reliable_targets) > 0:
            reliable_targets = torch.from_numpy(np.asarray(reliable_targets, np.float32)).to(device)
        else:
            reliable_targets = targets.new_zeros((0, 6))
    
        if len(uncertain_targets) > 0:
            uncertain_targets = torch.from_numpy(np.asarray(uncertain_targets, np.float32)).to(device)
        else:
            uncertain_targets = targets.new_zeros((0, 7))
    
        if len(uncertain_obj_targets) > 0:
            uncertain_obj_targets = torch.from_numpy(np.asarray(uncertain_obj_targets, np.float32)).to(device)
        else:
            uncertain_obj_targets = targets.new_zeros((0, 7))
    
        if len(uncertain_cls_targets) > 0:
            uncertain_cls_targets = torch.from_numpy(np.asarray(uncertain_cls_targets, np.float32)).to(device)
        else:
            uncertain_cls_targets = targets.new_zeros((0, 7))
    
        return reliable_targets, uncertain_targets, uncertain_obj_targets, uncertain_cls_targets

        
    def default_loss(self, p, targets):
        if getattr(self, "_dbg_printed", False) is False:
            print("ignore_obj=", self.ignore_obj,
                "pseudo_label_with_obj=", self.pseudo_label_with_obj,
                "pseudo_label_with_bbox=", self.pseudo_label_with_bbox,
                "pseudo_label_with_cls=", self.pseudo_label_with_cls,
                "uncertain_aug=", self.uncertain_aug)
            self._dbg_printed = True

        # device = targets.device
#=========将   # device = targets.device换成下述的==========================
        device = p[0].device
        targets = targets.to(device, non_blocking=True).float()
        
        # if not hasattr(self, "_dbg_dl_in_once"):
        #     self._dbg_dl_in_once = True
        #     print("[SSOD_LOSS.default_loss][IN] targets:",
                  # tuple(targets.shape), targets.dtype, targets.device,
                  # "p0:", p[0].device, "p0_shape:", tuple(p[0].shape))
        if (targets.shape[1] > 6) and (not hasattr(self, "_dbg_dl_in_once")):
            self._dbg_dl_in_once = True
            print("[SSOD_LOSS.default_loss][IN] targets:",
                  tuple(targets.shape), targets.dtype, targets.device,
                  "p0:", p[0].device, "p0_shape:", tuple(p[0].shape))

    #=========将   # device = targets.device换成上述的==========================
        lcls, lbox, lobj = torch.zeros(1, device=device), torch.zeros(1, device=device), torch.zeros(1, device=device)        
        if targets.shape[1] > 6:
            certain_targets, uc_targets, uc_obj_targets, uc_cls_targets = self.select_targets(targets)
            if not hasattr(self, "_dbg_dl_sel_once"):
                self._dbg_dl_sel_once = True
                print("[SSOD_LOSS.default_loss][AFTER_SELECT]",
                      "certain:", tuple(certain_targets.shape), certain_targets.device,
                      "uc:", tuple(uc_targets.shape), uc_targets.device)
                if certain_targets.numel() > 0:
                    mn = certain_targets[:, 2:6].min(0).values
                    mx = certain_targets[:, 2:6].max(0).values
                    print("[SSOD_LOSS.default_loss][AFTER_SELECT] xywh min/max:", mn.tolist(), mx.tolist())
            #  
            if self.uncertain_aug:
                tcls, tbox, indices, anchors  = self.assigner(p, certain_targets)  # targets
                if not hasattr(self, "_dbg_dl_asg_once"):
                    self._dbg_dl_asg_once = True
                    pos_per_layer = [len(ind[0]) for ind in indices]  # ind=(b,a,gj,gi)
                    print("[SSOD_LOSS.default_loss][ASSIGN] pos_per_layer:", pos_per_layer,
                          "sum_pos:", sum(pos_per_layer))
        ...
        if (targets.shape[1] > 6) and (not hasattr(self, "_dbg_final_once")):
            self._dbg_final_once = True
            print("[SSOD_LOSS.default_loss][FINAL] lbox/lobj/lcls:",
                  float(lbox.detach().cpu()), float(lobj.detach().cpu()), float(lcls.detach().cpu()))
        return loss * bs, loss_dict

 
    def __call__(self, p, targets):
        if not hasattr(self, "_dbg_path"):
            self._dbg_path = True
            print("ComputeStudentMatchLoss path:", "ota" if self.use_ota else "default")
        ...
        if not hasattr(self, "_dbg_once2"):
            self._dbg_once2 = True
            # print("targets.shape=", targets.shape)
            # print("ret loss=", loss.detach().item())
            # print("loss_dict=", {k: v.detach().item() for k,v in loss_dict.items()})
    ...
    def ota_loss(self, p, targets):
        ...
        lbox *= self.box_w #预测和实际boxIOU在哪里地地损失权重 这里是1 - CIoU 的均值
        lobj *= self.obj_w #有没有物体地损失权重BCEWithLogitsLoss  每一层都会算（哪怕没正样本）
        lcls *= self.cls_w
        bs = tobj.shape[0]  # batch size
