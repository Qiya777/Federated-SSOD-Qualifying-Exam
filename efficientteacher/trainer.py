...
class Trainer:
    ...
    def build_optimizer(self, cfg,optinit=True,weight_masks =None,ckpt=None):
        ...
        if cfg.linear_lr:
            ...
            self.lf = lambda x: (1 - x / max(self.epochs - 1, 1)) * (1.0 - cfg.hyp.lrf) + cfg.hyp.lrf  # linear
        else:
    ...
    def train(self, callbacks, val):
        ...
        self.best_fitness = 0
            #print("11111111111111111111111111111111111111111111111111111111111111111111111111111111111111111,start_epoch:",self.start_epoch, self.epochs)
        for self.epoch in range(self.start_epoch, self.epochs):  
            #print("2222222222222222222222222222222222222222222222222222222,epoch",self.epoch)
            if self.epoch == self.break_epoch:
                break
            self.before_epoch()
            self.train_in_epoch(callbacks)
            each_epoch_results=self.after_epoch(callbacks, val)
            # print("epoch:",self.epoch,"results:",each_epoch_results)


        # ---- 写 results.csv ----
            if self.RANK in [-1, 0] and each_epoch_results is not None:
                csv_file = self.save_dir / 'results.csv'
                header = 'epoch,P,R,mAP50,mAP50_95,box_loss,obj_loss,cls_loss\n'
                # 如果是第一次写，先写表头
                if not csv_file.exists():
                    with open(csv_file, 'w') as f:
                        f.write(header)
                # each_epoch_results 对应: (P, R, mAP@0.5, mAP@0.5:0.95, box, obj, cls)
                p, r, map50, map5095, box_loss, obj_loss, cls_loss = each_epoch_results
                with open(csv_file, 'a') as f:
                    f.write(
                        f'{self.epoch},{p:.6f},{r:.6f},{map50:.6f},{map5095:.6f},'
                        f'{box_loss:.6f},{obj_loss:.6f},{cls_loss:.6f}\n'
                    )
        # ---- csv 写入结束 ----
